/*-----------------------------------------------------------------------------
 * Umicom Platform
 * File: src/commands/cmd_framework_audit.c
 *
 * PURPOSE:
...
int umi_cmd_framework_audit(void) {
    ...
    UmiFrameworkAuditSummary s;
    r = umi_platform_framework_audit(&w, &s);
    if (!umi_platform_result_is_ok(r)) { fprintf(stderr, "%s\n", r.message); return 1; }
    umi_platform_report_framework_audit(&s);
    return s.mismatched == 0u ? 0 : 2;
}
