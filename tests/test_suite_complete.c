/*-----------------------------------------------------------------------------
 * Umicom Platform
 * File: tests/test_suite_complete.c
 *
 * PURPOSE:
...
int main(void) {
    ...
    UMI_TEST_ASSERT(umi_platform_result_is_ok(
        umi_platform_suite_load("manifests/suites/complete.json",&s)));
    UMI_TEST_ASSERT(s.enabled);
    UMI_TEST_ASSERT(s.required_member_count==6U);
    UMI_TEST_ASSERT(s.optional_member_count==6U);
    return 0;
}
